...
namespace QuantSA.ExcelFunctions
{
    public class XLCredit
    {
        ...
        public static double GetSurvivalProb(
            [QuantSAExcelArgument(Description = "The hazard rate curve or other source of default probabilities.")]ISurvivalProbabilitySource survivalProbabilitySource,
            [QuantSAExcelArgument(Description = "If date2 is ommitted the date until which survival is calculated.  If date2 is provided the date from which survival is calculated.")]Date date1,
            [QuantSAExcelArgument(Description = "Optional: If provided then the survival probability is calculated from date1 until date2.", Default = "null")]Date date2)
        {
            if (date2 == null)
                return survivalProbabilitySource.GetSP(date1);
            else
                return survivalProbabilitySource.GetSP(date1, date2);                
        }
